...
elseif any(losses < 0) || any(inputs < 0)
    ...
    fig1 = figure('color','white','tag','sankeyDiagram','WindowState','maximized');
...
%     fontsize = min(16, 10 + ceil((frInputs(1)-0.05)/0.025));
    ...
    text(0, frInputs(1)/2, inputLabel, 'FontSize', 18,'HorizontalAlignment','right','Rotation',0);
...
%                 fullLabel = sprintf('%s\n%.1f [%s] %.1f [%%]', labels{j}, inputs(j), unit, 100*frInputs(j));
            ...
            elseif frInputs(j) > 0.05
                ...
                fullLabel = sprintf('%s: %.2f [%s]', labels{j}, inputs(j), unit);
...
%                 fullLabel = sprintf('%s: %.1f [%s] %.1f [%%]',labels{j}, inputs(j), unit, 100*frInputs(j));
            ...
            %draw input label%
            text(xText, yText, fullLabel, 'FontSize', 18,'HorizontalAlignment','right');
    ...
    for i = 1 : length(losses)
        ...
        if frLosses(i) > eps
            ...
            if frLosses(i) > 0.1
                ...
                fullLabel = sprintf('%s\n%.2f [%%]',labels{i+length(inputs)}, 100*frLosses2(i));
%                 fullLabel = sprintf('%s\n%.1f [%%]',labels{i+length(inputs)}, 100*frLosses(i));
            ...
            elseif frLosses(i) > 0.05
                ...
                fullLabel = sprintf('%s: %.2f [%%]',labels{i+length(inputs)}, 100*frLosses2(i));
            ...
            %draw losses label%
            text(txtX, txtY, fullLabel, 'Rotation', 90, 'FontSize', 18);
%             text(txtX, txtY, fullLabel, 'Rotation', 90, 'FontSize', fontsize);
    ...
    endText = sprintf('%s\n%.2f [%s]',labels{length(losses)+length(inputs)+1}, outputFinal, unit);
%     endText = sprintf('%s\n%.0f [%s] %.1f [%%]',labels{length(losses)+length(inputs)+1}, outputFinal, unit,100*outputFinal/inputFinal);
    ...
    text(newPos + 0.05, (limTop+limBot)/2, endText, 'FontSize', 18);
